...
/*   Updated: 2025/02/26 19:05:17 by ccarro-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/so_long.h"

void	remove_player_collectibles(mlx_t *mlx, mlx_image_t *player_img,
...
void	render_player_collectibles(t_map map, t_images *textures, mlx_t *mlx)
{
	...
	while (y < map.height)
	{
		...
		{
			if (map.grid[y][x] == 'P')
				mlx_image_to_window(mlx, textures->player_image, x * 64, y
					* 64);
			if (map.grid[y][x] == 'C')
				mlx_image_to_window(mlx, textures->collectible_image, x * 64, y
					* 64);
			x++;
		}
		y++;
...
{
	remove_player_collectibles(mlx, textures->player_image,
		textures->collectible_image);
