...
void	elements_checker(char **map_grid)
{
	...
	while (map_grid[i])
	{
		...
		while (map_grid[i][j])
		{
			if (map_grid[i][j] != '1' && map_grid[i][j] != '0'
				&& map_grid[i][j] != 'P' && map_grid[i][j] != 'C'
				&& map_grid[i][j] != 'E' && map_grid[i][j] != '\n')
			{
				print_invalid_element(i, j, map_grid[i][j]);
			}
			j++;
		}
		i++;
...
void	flood_fill(char **map_copy, size_t player_x, size_t player_y,
		t_map *map)
{
	// Dentro del mapa
	if (player_x <= 0 || player_x >= map->width || player_y <= 0
		...
		return ;
	// Espacio válido
	if (map_copy[player_y][player_x] == '1'
